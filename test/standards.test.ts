...
describe("apply and check", () => {
  ...
  it("leaves README branding to an explicit markdown-themer owner", () => {
    ...
    expect(isGeneratedReadme(readRepoMeta(cwd))).toBe(true);
  });

  it("delegates native mdtheme ownership in a Rust repository without Node files", () => {
    const cwd = createFixtureRepo();
    ...
    writeFileSync(join(cwd, "README.md"), output);
    writeFileSync(join(cwd, "mdtheme.yaml"), "themes: []\n");
    expect(runApply(cwd, YEAR).map((change) => change.path)).not.toContain("README.md");
    expect(readFileSync(join(cwd, "README.md"), "utf8")).toBe(output);
    ...
    writeFileSync(join(cwd, "mdtheme.yml"), "themes: []\n");
    expect(() => runApply(cwd, YEAR)).toThrow(/multiple supported configs/);
    rmSync(join(cwd, "mdtheme.yml"));
    rmSync(join(cwd, "mdtheme.yaml"));
    symlinkSync("README.md.src", join(cwd, "mdtheme.yaml"));
    expect(() => runApply(cwd, YEAR)).toThrow(/regular non-symlink/);
    expect(readFileSync(join(cwd, "README.md"), "utf8")).toBe(output);
  });
